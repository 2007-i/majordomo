   ...
   while(!$sink->terminated) 
   {
      ...
      if ($old_message != $latest_message) 
      {
         ...
         if (isset($tmp['IMPORTANCE']) && $tmp['IMPORTANCE']>0) 
         {
            ...
            for($i=0;$i<$total;$i++) 
            {
               $message = Core_Convert::Utf8ToCp1251($latest_message);
               echo "Sending to " . $users[$i]['SKYPE'] . ": " . convert_cyr_string($message, 'w', 'd') . "\n";
               $skype->SendMessage(trim($users[$i]['SKYPE']), $message);
            }
         }
      }
      ...
      { 
         if ($Status == 0 || $Status == 2) 
         {
            ...
            if ($pMessage->FromHandle <> $CurrentUser->Handle ) 
            {
               ...
               $qrys  = explode(' '.DEVIDER.' ', Core_Convert::Cp1251ToUtf8($pMessage->Body));
               $total = count($qrys);
